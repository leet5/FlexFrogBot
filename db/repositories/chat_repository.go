...
func (r *chatRepository) Create(ctx context.Context, chat *domain.Chat) error {
	var id int64
	query := `
		INSERT INTO chats (id, title, username, thumbnail, watched, is_private)
		VALUES ($1, $2, $3, $4, $5, $6)
		RETURNING id
	`
	if err := r.db.QueryRowContext(ctx, query, chat.Id, chat.Title, chat.Username, chat.Thumbnail, chat.Watched, chat.IsPrivate).Scan(&id); err != nil {
		return err
	}
	return nil
}
...
func (r *chatRepository) GetByID(ctx context.Context, chatID int64) (*domain.Chat, error) {
	...
	query := `
		SELECT id, title, username, thumbnail, watched, is_private
		FROM chats
		WHERE id = $1
	`
	err := r.db.QueryRowContext(ctx, query, chatID).Scan(&chat.Id, &chat.Title, &chat.Username, &chat.Thumbnail, &chat.Watched, &chat.IsPrivate)
	if err != nil {
